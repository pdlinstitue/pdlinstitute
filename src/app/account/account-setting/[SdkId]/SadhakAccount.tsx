...
interface SadhakAccountProps {
    sdkAccountById:SadhakAccountItems;
}


const SadhakAccount: React.FC<SadhakAccountProps> = ({ sdkAccountById }) => {

  const router = useRouter();
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState<boolean>(false);
  const [errorMessage, setErrorMessage] = useState<string>("");
  const [contactDetails, setContactDetails] = useState<SadhakAccountItems>(sdkAccountById);
    ...
    try {
      if (!contactDetails.sdkPhone.trim()) {
        ...
        const response = await fetch(
          `${BASE_API_URL}/api/users/${contactDetails._id}/update-contact`,
          {
            method: "PUT",
            body: JSON.stringify({
