...
const MyAttendance : React.FC = () => {
    ...
    { header: 'Action', accessorKey: 'action', 
      cell: ({ row }: { row: any }) => ( 
        <div className='flex items-center gap-3'> 
          <button type='button' title='My Classes' onClick={()=> router.push(`/account/my-attendance/${row.original._id}/my-classes`)} className='text-orange-500 border-[1.5px] border-orange-700 p-1 rounded-full  hover:border-black'><PiChalkboardTeacher size={12}/></button>      
        </div> 
      ), 
    }
  ], []);
  ...
  useEffect(() => {
    async function fetchEnrolledBatchesBySdkId() {
      try {
        const res = await fetch(`${BASE_API_URL}/api/my-attendance?sdkId=${Cookies.get("loggedInUserId")}`, { cache: "no-store" });
