...
const MarkAttendance : React.FC<IAtdParams> = ({params}) => {

  const router = useRouter();
  const {BthId} = use(params);
  const {ClsId} = use(params);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [enrollData, setEnrollData] = useState<MarkAttendanceProps[] | null>([]);
  ...
  const columns = React.useMemo(() => [
    ...
    { header: 'Presnt', accessorKey: 'isPresent'},
    { header: 'Remarks', accessorKey: 'absRemarks'},
    { header: 'Action', accessorKey: 'atdAction', 
        cell: ({ row }: { row: any }) => ( 
          <div className='flex items-center justify-center'> 
            <button type='button' title='View' onClick={()=> router.push(`/account/attendance-list/${row.original._id}/view-class`)} className='text-green-500 border-[1.5px] border-green-700 p-1 rounded-full hover:border-black'><FiEye size={12}/></button>
          </div> 
        ), 
      },
  ], []);
    ...
    async function fetchBatchData() {
      try {
        const res = await fetch(`${BASE_API_URL}/api/enrollment-by-batch-id/${BthId}`, { cache: "no-store" });
        const attendData = await res.json();
        
        let enrollList: MarkAttendanceProps[] = attendData.enrByBatchId?.flatMap((item: any) => {          
          return {
            _id: item._id,
            clsId: ClsId, 
            sdkFstName: item.sdkFstName,
            sdkPhone: item.sdkPhone || "", 
            isPresent: false,
            absRemarks:""
          }
        });
