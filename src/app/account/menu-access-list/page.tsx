...
const MenuAccessList : React.FC = () => {

  const router = useRouter();
  const [accMenuData, setAccMenuData] = useState<MenuAccessListProps[] | null>([]);
  ...
  const columns = React.useMemo(() => [ 
    ...
    useEffect(() => {
      async function fetchSideMenuData() {
        try {
          const res = await fetch(`${BASE_API_URL}/api/menu-access-list`, { cache: "no-store" });
          const menuData = await res.json();
          setAccMenuData(menuData?.accessList);
        } catch (error) {
  ...
  return (
    <div>
      <div className='flex items-center justify-between mb-4'>
        <Link href="/account/add-new-access" title='Create Access' className='btnLeft'>Create Access</Link>
