...
const BatchList : React.FC = () => {
  ...
  const columns = React.useMemo(() => [
    ...
    { header: 'Start Date', accessorKey: 'bthStart'},
    { header: 'End Date', accessorKey: 'bthEnd'},
    { header: 'Course', accessorKey: 'bthCourse'},
    { header: 'Mode', accessorKey: 'bthMode'},
    { header: 'Language', accessorKey: 'bthLang'},
    ...
    { header: 'Action', accessorKey: 'coAction', 
      cell: ({ row }: { row: any }) => ( 
        <div className='flex items-center gap-3'> 
          <button type='button' title='View' onClick={()=> router.push(`/account/batchlist/${row.original._id}/view-batch`)} className='text-green-500 border-[1.5px] border-green-700 p-1 rounded-full hover:border-black'><FiEye size={12}/></button>
          <button type='button' title='Edit' onClick={()=> router.push(`/account/batchlist/${row.original._id}/edit-batch`)} className='text-orange-500 border-[1.5px] border-orange-700 p-1 rounded-full  hover:border-black'><BiEditAlt size={12}/></button>
          <button type='button' title='Delete' onClick={()=> router.push(`/account/batchlist/${row.original._id}/delete-batch`)} className='text-red-500 border-[1.5px] border-red-700 p-1 rounded-full  hover:border-black'><RxCross2 size={12}/></button>
        </div> 
      ), 
    },
  ], []);
  ...
  async function fetchBatchData() {
    try {
      ...
      setBatchData(batchData.bthList);
    } catch (error) {
