    ...
    {header: 'Action', accessorKey: 'action', 
      cell: ({ row }: { row: any }) => ( 
        <div className='flex items-center gap-3'> 
          <button type='button' title='Edit' onClick={()=> router.push(`/account/my-docs/id-card/${row.original._id}/edit-id-card`)} className='text-orange-500 border-[1.5px] border-orange-700 p-1 rounded-full  hover:border-black'><BiEditAlt size={12}/></button>
        </div> 
      ), 
    }, 
    ...
    <div>
      <div>
        <div className='flex mb-2 items-center justify-between'>
          ...
          {data?.length === 0 && (
            <button type='button' onClick={()=> router.push("/account/add-new-id")} title="Upload ID" className="btnLeft">
              Upload ID
            </button>)
          }
