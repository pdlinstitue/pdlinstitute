...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { clsList: clsListWithCounts, success: true },
      { status: 200 }
    );
  } catch (error) {
    console.error("Error while fetching clsData:", error);
    return new NextResponse("Error while fetching clsData: " + error, {
      status: 500,
    });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!bthId || !corId) {
      return NextResponse.json(
        { success: false, msg: "Batch and Course are required." },
        { status: 400 }
      );
    }
    ...
    await newClass.save();
    return NextResponse.json(
      { success: true, msg: "Classes created successfully", class: newClass },
      { status: 201 }
    );
  } catch (error: any) {
