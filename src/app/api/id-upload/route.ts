...
export async function GET(req: NextRequest) {
    ...
    const filePath = path.resolve(`${UPLOAD_PATH}/id-images`, name.replace("/id-images/", ""));
...
export async function POST(req: NextRequest) {
  try {
      ...
      const fileName = formData.get("idImageFileName")?.toString() || "";
      ...
      let uniqueName = fileName?.split("/").pop() || `idImage_${Date.now()}.jpeg`;
      if (!uniqueName || uniqueName === "undefined" || uniqueName === "null") {
          uniqueName = `idImage_${Date.now()}.jpeg`;
      }
      ...
      const filePath = path.resolve(`${UPLOAD_PATH}/id-images`, uniqueName);
      ...
      const imageUrl = `/id-images/${uniqueName}`;
