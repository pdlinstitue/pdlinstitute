...
export async function GET(req: NextRequest) {
    ...
    const filePath = path.resolve(`${UPLOAD_PATH}/prc-images`, name.replace("/prc-images/", ""));
...
export async function POST(req: NextRequest) {
  try {
      ...
      let uniqueName = fileName?.split("/").pop() || `prcImage_${Date.now()}.jpeg`;
      if (!uniqueName || uniqueName === "undefined" || uniqueName === "null") {
          uniqueName = `prcImage_${Date.now()}.jpeg`;
      }
      ...
      const filePath = path.resolve(`${UPLOAD_PATH}/prc-images`, uniqueName);
