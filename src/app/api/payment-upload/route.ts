...
export async function POST(req:NextRequest) {
    try {
        ...
        const uniqueName = (fileName ? fileName.split('/').pop() : `paymentImage_${Date.now()}.jpeg`) || `profileImage_${Date.now()}.jpeg`;
        const filePath = join(process.cwd(), "public/payment-images", uniqueName);
