...
export async function GET(req: NextRequest) {
    ...
    const filePath = path.resolve(`${UPLOAD_PATH}/adds-images`, name.replace("/adds-images/", ""));
...
export async function POST(req: NextRequest) {
  try { 
      ...
      let uniqueName = fileName?.split("/").pop() || `addsImage_${Date.now()}.jpeg`;
      if (!uniqueName || uniqueName === "undefined" || uniqueName === "null") {
          uniqueName = `addsImage_${Date.now()}.jpeg`;
      }
      ...
      const filePath = path.resolve(`${UPLOAD_PATH}/adds-images`, uniqueName);
