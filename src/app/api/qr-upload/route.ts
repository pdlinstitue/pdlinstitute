...
export async function GET(req: NextRequest) {
    ...
    const filePath = path.resolve(`${UPLOAD_PATH}/qr-images`, name.replace("/qr-images/", ""));
...
export async function POST(req: NextRequest) {
  try {
      ...
      let uniqueName = fileName?.split("/").pop() || `qrImage_${Date.now()}.jpeg`;
      if (!uniqueName || uniqueName === "undefined" || uniqueName === "null") {
          uniqueName = `qrImage_${Date.now()}.jpeg`;
      }
      ...
      const filePath = path.resolve(`${UPLOAD_PATH}/qr-images`, uniqueName);
