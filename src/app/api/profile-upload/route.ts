...
export async function GET(req: NextRequest) {
    ...
    const filePath = path.resolve(`${UPLOAD_PATH}/profile-images`, name.replace("/profile-images/", ""));
...
export async function POST(req: NextRequest) {
  try {
      ...
      const fileName = formData.get("profileImageFileName")?.toString() || "";
      ...
      let uniqueName = fileName?.split("/").pop() || `profileImage_${Date.now()}.jpeg`;
      if (!uniqueName || uniqueName === "undefined" || uniqueName === "null") {
          uniqueName = `profileImage_${Date.now()}.jpeg`;
      }
      ...
      const filePath = path.resolve(`${UPLOAD_PATH}/profile-images`, uniqueName);
      ...
      const imageUrl = `/profile-images/${uniqueName}`;
