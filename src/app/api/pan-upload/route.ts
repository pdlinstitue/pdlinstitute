...
export async function POST(req:NextRequest) {
    try {
        ...
        const uniqueName = (fileName ? fileName.split('/').pop() : `panImage_${Date.now()}.jpeg`) || `panImage_${Date.now()}.jpeg`;
        ...
        // Revalidate cache
        revalidatePath("/");
        return NextResponse.json({ success: true, imageUrl, msg:"Pan upload successfull." });
