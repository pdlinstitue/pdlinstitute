...
export async function GET(req: NextRequest) {
    ...
    const filePath = path.resolve(`${UPLOAD_PATH}/pan-images`, name.replace("/pan-images/", ""));
...
export async function POST(req: NextRequest) {
  try {
      ...
      const file = formData.get("panImage");
      const fileName = formData.get("panImageFileName")?.toString() || "";
      ...
      let uniqueName = fileName?.split("/").pop() || `panImage_${Date.now()}.jpeg`;
      if (!uniqueName || uniqueName === "undefined" || uniqueName === "null") {
          uniqueName = `panImage_${Date.now()}.jpeg`;
      }
      ...
      const filePath = path.resolve(`${UPLOAD_PATH}/pan-images`, uniqueName);
      ...
      const imageUrl = `/pan-images/${uniqueName}`;
